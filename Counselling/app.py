...
def print_report():
    cur = connection.cursor()
    cur.execute("SELECT * FROM appointment WHERE action='Approved' AND counsellor=%s",(session['username'],))
    data = cur.fetchall()
    cur.close()
    base_url = request.base_url
    return render_template('report.html', data=data, base_url=base_url)
